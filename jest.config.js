...
module.exports = {
	...
	// and makes tests stable in CI while leaving local behavior unchanged.
	if(process.env.GITHUB_ACTIONS || process.env.CI) {
	module.exports.moduleNameMapper = module.exports.moduleNameMapper || {};
	module.exports.moduleNameMapper['^mongodb-memory-server$'] = '<rootDir>/test-mocks/mongodb-memory-server.mock.ts';
}
// Ignore built artifacts to avoid duplicate manual mock detection
modulePathIgnorePatterns: ['<rootDir>/dist/'],
	testPathIgnorePatterns: ['<rootDir>/src/scripts/'],
