...
const brInProgress = new Map<string, boolean>();
	...
	execute: async (channel: string, user: string, args: string[], _text: string, _msg: ChatMessage) => {
		...
		try {
			...
			async function addSimulatedBots(count: number, participantsObj: Record<string, PlayerState>, botSet: Set<string>) {
				const added: string[] = [];
				let idx = 1;
				while (Object.keys(participantsObj).length < count + 1) {
					const candidate = TEST_BOT_NAMES[(idx - 1) % TEST_BOT_NAMES.length];
					let botName = candidate;
					let suffix = 1;
					while (participantsObj[botName]) {
						botName = `${candidate}${suffix}`;
						suffix++;
					}
					participantsObj[botName] = { name: botName, health: 100, xp: 0, level: 0, roundsSurvived: 0 };
					added.push(botName);
					idx++;
			...
			if (process.env.ENVIRONMENT === 'dev' && requestedBots > 0) {
				...
				await addSimulatedBots(addCount, participants, simulatedBotSet);
			} else if (process.env.ENVIRONMENT === 'dev' && requestedBots === 0) {
				const defaultAdd = 4;
				await addSimulatedBots(defaultAdd, participants, simulatedBotSet);
			}
