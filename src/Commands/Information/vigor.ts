	...
	execute: async (channel: string, user: string, args: string[], text: string, msg: PrivateMessage) => {
		try {
			...
			case 'about':
				...
				break;
			case 'lore':
				chatClient.say(channel, 'theres no written lore for the game, its all recorded on tapes in the game itself, i will eventually, listen to them and write out what i can do describe the lore of the game as twitch messages can only be a max of 500 characters.');
				break;
			case 'item':// Guess of health data for healing consumables
				...
				break;
			default:
				await chatClient.say(channel, `${display}, Usage: ${vigor.usage}`);
				break;
			}
		} catch (error: any) {
