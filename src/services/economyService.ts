...
export async function deposit(userId: string, amount: number, session?: mongoose.ClientSession) {
    ...
    if (session) {
        ...
        await TransactionLog.create([{ type: 'deposit', to: userId, amount, meta: {}, }], opts);
    ...
    await TransactionLog.create([{ type: 'deposit', to: userId, amount, meta: {}, }]);
    return acct as IBankAccount;
}

export async function withdraw(userId: string, amount: number, session?: mongoose.ClientSession) {
    if (amount <= 0) throw new EconomyError('Amount must be positive');
    const opts = session ? { session } : undefined as any;
    if (session) {
        ...
        acct.balance -= amount;
        await acct.save(opts);
        await TransactionLog.create([{ type: 'withdraw', from: userId, amount }], opts);
        return acct as IBankAccount;
    }
    ...
    await TransactionLog.create([{ type: 'withdraw', from: userId, amount }]);
    return acct as IBankAccount;
}
...
export async function transfer(from: string, to: string, amount: number) {
    ...
    if (await transactionsSupported()) {
        ...
        try {
            ...
            await TransactionLog.create([{ type: 'transfer', from, to, amount }], { session });
    ...
    await TransactionLog.create([{ type: 'transfer', from, to, amount }]);
}
